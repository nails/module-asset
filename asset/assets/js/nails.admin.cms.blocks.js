...
NAILS_Admin_CMS_Blocks = function()
{
	...
	this._type_changed = function()
	{
		...
		switch( _type )
		{
			...
			case 'richtext' :
				...
				$( '#default-value' ).show();
				$( '#ckeditor-warn' ).show();
				CKEDITOR.replace( 'default_value', { 'customConfig' : window.NAILS.URL + 'js/ckeditor.config.min.js' } );
	...
	this._add_translation = function()
	{
		...
		{
			CKEDITOR.replace( 'translation_' + this.new_counter, { 'customConfig' : window.NAILS.URL + 'js/ckeditor.config.min.js' } );
		}
